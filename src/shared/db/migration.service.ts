...
import { chatDB, ChatRecord, MessageRecord } from './chat.db';

const CHATS_STORAGE_KEY = 'ai-chat-chats';
const CURRENT_MODEL_STORAGE_KEY = 'ai-chat-current-model';
const CURRENT_MODEL_SETTING_KEY = 'current-model';

type ChatStateValue = 'idle' | 'thinking' | 'error';
...
export class MigrationService {
  ...
  async migrateIfNeeded(): Promise<void> {
    ...
    await chatDB.transaction('rw', chatDB.chats, chatDB.messages, chatDB.settings, async () => {
      ...
      if (modelFromLegacy !== null) {
        const existingModel = await chatDB.settings.get(CURRENT_MODEL_SETTING_KEY);
        if (!existingModel) {
          await chatDB.settings.put({
            key: CURRENT_MODEL_SETTING_KEY,
  ...
  private normalizeChatState(value: unknown): ChatStateValue {
    if (value === 'error') {
      return 'error';
    }

    if (value === 'idle' || value === 'thinking') {
      return 'idle';
    }
  ...
  private normalizeMessageRole(value: unknown): MessageRoleValue {
    if (value === 'assistant') {
      return 'assistant';
    }
